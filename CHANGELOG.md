...
- **`rayspec deploy --dry-run` reports a frontend-only document truthfully instead of `ok: false`.**
  ...
  untouched, it stays as fast as it was (composing a product document loads no boot machinery for the
  new classification), and the guards are unchanged (`--apply-migration` with `--dry-run`, and
