...
- **`rayspec deploy --dry-run` reports a frontend-only document truthfully instead of `ok: false`.**
  ...
  exit `1` for a document the same command **boots** on the static branch. The dry-run now classifies
  the document with the same detection that boot branches on, before it parses anything, and answers
  for the boot it would perform: `ok: true` and exit `0`, with a `staticProfile` block in place of
  `composed` naming the profile, listing the `frontendMounts` it would serve, and stating outright
  that no database is touched, no migration applies, and there is nothing to compose. The honest
  boundary narrows with it — only the document was read, so what stays unproven is whether the
  ...
  untouched, and the guards are unchanged (`--apply-migration` with `--dry-run`, and
  `--apply-migration` against a frontend-only document, are both still refused as usage errors).
