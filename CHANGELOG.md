...
- **Cancelling a run on the `anthropic` backend is now described by how long it actually takes, and
  ...
  abort, a child that honours `SIGTERM` was gone at 2009 ms, and one that ignores it at 7010 ms,
  while the adapter's own call returned at about 2005 ms — so a caller gets its answer while the
  child may still be exiting. Both escalation timers are `unref()`ed, which costs something real: a
  host process that exits inside that window loses the `SIGKILL` rung, and a child that ignores
  `SIGTERM` is then left running — measured still alive ten seconds after a host that exited 200 ms
  after the abort. The adapter's README now lists that limit alongside the four others this backend
  inherits (every rung is sent to the child's own process id and never to a process group; no signal
  reaches a run executing in a separate worker process, tracked as #210; a tool call already in
  flight is not interrupted; work already committed upstream is not undone), and the reference
  table's `anthropic` row says seconds rather than implying instants. Nothing about how a run
  behaves changed — what changed is that a reader can now find out what they are getting, and that a
  test in the adapter package drives the real SDK against a stand-in executable, holds the process id
  the SDK spawned and watches it disappear, instead of checking that a boolean flipped. That test
  exercises the SDK's process-level teardown; it says nothing about the vendor binary's own shutdown
  behaviour.
