...
| `workforce.approval.requested` | task | `approvalId`, `taskId`, `question`, `options`, `approver`, `onTimeout` (+ `escalatedFrom` when the request is the escalation of a timed-out approval; + `policy: true` and `policyId` when a declared `approvalPolicies` rule IMPOSED the approval by intercepting a completion, rather than a seat asking for one — the question is then engine-composed and names the rule) |
| `workforce.approval.decided` | task | `approvalId`, `taskId`, `decision`, `decidedBy` (the VERIFIED principal — the route derives it from the credential), `reason`, `overriddenApprover` (PRESENT ONLY on a break-glass decision: the row named this principal, someone holding `workforce:override` decided it anyway, and this field is the engine saying so rather than leaving `decidedBy` to silently contradict the recorded approver), `gatedCompletion: true` (PRESENT ONLY when the decision released or withheld a stored, FINISHED result — an `approve` completes the task then and there, a `reject` parks it for a human; a decision without this field answered a question the seat asked and merely woke the task) |
...
| `workforce.review.decided` | task | `reviewId`, `taskId`, `reviewer`, `round`, `verdict`, `decidedBy`, `reasons`, `requiredChanges`, `outcome` (what the verdict did to the task), `overriddenReviewer` (PRESENT ONLY on a break-glass verdict — the one case where `decidedBy` does not answer to `reviewer`) |
