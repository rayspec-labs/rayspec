...
- **A rule guarding a label no employee holds is now a lint ERROR, not a warning**
  (`packages/kernel/spec/src/workforce-lint.ts:691`). A clause that can never fire is a typo that
...
(`packages/kernel/spec/src/workforce-lint.ts:230`) and again at dispatch composition
...
fate at all (`packages/kernel/spec/src/workforce-lint.ts:637` guards the fate,
`packages/kernel/spec/src/workforce-lint.ts:644` raises it as an error). That is a **different**
...
packages/kernel/spec/src/workforce-lint.ts:230 | if (tool !== undefined && isReservedWorkforceToolSpelling(tool.name)) {
packages/kernel/spec/src/workforce-lint.ts:637 | if (approval.onTimeout !== 'escalate') return;
packages/kernel/spec/src/workforce-lint.ts:644 | 'invalid_orchestrator',
packages/kernel/spec/src/workforce-lint.ts:691 | 'workforce_label_unheld',
