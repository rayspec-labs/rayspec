...
2. **A supplied `Idempotency-Key` is refused, not ignored**
   (`packages/compose/api-auth/src/routes/workforce.ts:1126`). This route mints a fresh billed root
   per call; silently dropping the header would be a lost-write trap.
3. **Rate limit before the body read** (`packages/compose/api-auth/src/routes/workforce.ts:1138`),
...
7. `requestedBy` is stamped from the verified principal
   (`packages/compose/api-auth/src/routes/workforce.ts:1156`), never read from the body.
...
2. the principal must **hold** `workforce:override`
   ...
   and run **before** the engine call (`packages/compose/api-auth/src/routes/workforce.ts:1011`) so
...
on every mutating route (`packages/compose/api-auth/src/routes/workforce.ts:1017` on the
approval-decide route, `packages/compose/api-auth/src/routes/workforce.ts:1091` on the
...
2. `decideApproval` has exactly one production caller, the HTTP route
   (`packages/compose/api-auth/src/routes/workforce.ts:1013`). There is no in-engine approval
...
packages/compose/api-auth/src/routes/workforce.ts:1011 | const override = await breakGlassAuthorized(deps, c, body.override);
packages/compose/api-auth/src/routes/workforce.ts:1013 | const approval = await decideApproval(tdb, {
packages/compose/api-auth/src/routes/workforce.ts:1017 | decidedBy: actorFrom(c),
packages/compose/api-auth/src/routes/workforce.ts:1091 | actor: actorFrom(c),
packages/compose/api-auth/src/routes/workforce.ts:1126 | if (c.req.header('Idempotency-Key') !== undefined) {
packages/compose/api-auth/src/routes/workforce.ts:1138 | const { allowed, retryAfterMs } = await deps.rateLimiter.checkAsync(
packages/compose/api-auth/src/routes/workforce.ts:1156 | requestedBy: actorFrom(c),
