...
| Tier            | Packages                                                                    | Role |
| --------------- | --------------------------------------------------------------------------- | ---- |
| **kernel**      | `core`, `spec`, `db`, `auth-core`, `platform`, `handler-sdk`, `pack-sdk`, `stt-port` | The neutral types, the spec grammar + parser, the tenant-scoped data layer, the auth primitives, the platform assembly, the handler authoring SDK, the type-only contract an out-of-tree extension pack compiles against, and the neutral speech-to-text port (the `SttAdapter` contract, registry, media-resolution seam, and fake adapter). |
