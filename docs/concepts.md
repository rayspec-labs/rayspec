...
has a secure default when unset: a CSP baseline of
`default-src 'self'; frame-ancestors 'none'; object-src 'none'; base-uri 'self'` (same-origin,
deliberately with no `'unsafe-inline'`), and a Permissions-Policy of `camera=(), microphone=(),
geolocation=()`. A deployer can override either — the value given replaces that whole baseline
