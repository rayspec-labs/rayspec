...
leading BOM) is stripped while interior bytes are preserved — and when that strip
actually changes a secret, the boot says so once, naming the variable and the kind
of change, never the value.
