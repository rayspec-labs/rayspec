 ...
 * location upward — `dist/` shipped alone resolves nothing. IN THIS REPO that `node_modules` is the pnpm
 * workspace link; out of the repo it is a real install of the RELEASED `@rayspec/platform`, and the pack
 * DIRECTORY (`dist/` + `node_modules/`) is what has to reach the deploy target — see README.md,
 * 'Shipping this pack from its own repo'. Deploy a spec that references the built pack directory
 * `packs/stream-pack/dist` (the loader resolves the compiled `.js` — the manifest keeps its authored
 * `.ts` module paths, and `.js`-preferred resolution loads the compiled siblings, so no manifest rewrite
 * is needed). Run: `node examples/stream-backend/build.mjs`.
 */
