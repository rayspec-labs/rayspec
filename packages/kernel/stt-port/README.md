...
The provider-neutral speech-to-text port: the `SttAdapter` contract and neutral
transcript types, the adapter registry, the media-resolution seam, the shared transcript
normalizer, and a deterministic fake adapter for tests. Concrete providers (e.g.
`@rayspec/adapter-deepgram`) depend on this port.
