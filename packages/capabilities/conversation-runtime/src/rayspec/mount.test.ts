 ...
 * POST `{kind:'handler'}` turn submit), the resolved handler map INCLUDING the S3 tx-posture flag
 * (turn submit = `routeTx: 'handler-managed'`, create = default) — plus the HTTP-facing behavior
 * of the bound handlers: the S3 turn flow (intake + REAL reply in one request), `httpResponse`
...
function harness(sink: TurnSubmittedSink, capability?: ConversationCapabilityConfig): Harness {
  ...
  return {
    ...
    mounted: mountConversationCapability({
      ...
      // FQ-1: an optional capability override (threaded into `mountConversationCapability`, which
...
/**
 * FQ-1 — a permissive `messageIdPattern` that ADMITS raw CR/LF (so hostile framing bytes can ride the
...
/**
 * FQ-1 — a hostile message_id engineered to break a naive `data: <text>` frame: an LF, a blank line,
 ...
 * (the default pattern blocks CR/LF); this pins the framing invariant on the two builders SS-1 drove
...
describe('mountConversationCapability — the composable fragments', () => {
  it('returns BOTH stores, BOTH routes as whole tuples, and the route-kind handler map', () => {
    ...
    // THE S3 TX-POSTURE TUPLE (whole-invariant): the TURN entry runs handler-managed (the engine
...
describe('mountConversationCapability — the bound handlers (create → turn → REAL reply)', () => {
  it('creates then submits a turn: the S3 response = intake facts + the reply block; assistant row persisted; ONE event', async () => {
    ...
    const turn = (await driveTurn(h, 'c-1', { message_id: 'm-1', text: 'hello there' })) as Record<
      ...
      unknown
    >;
    // The S1/S2 intake facts survive as a SUPERSET (arm-compat: existing consumers keep working).
  ...
  it('maps a sink ConversationEventRejectedError to the clean 403 with ZERO model work (commit-then-403)', async () => {
    ...
    // COMMIT-THEN-403 (the preserved S2 semantics): the turn row COMMITTED (persist-then-emit is
...
describe('mountConversationCapability — the S4 SSE streaming egress (content-negotiated)', () => {
  ...
  it('content-negotiation JSON path: no Accept → the S3 JSON result (byte-identical); a malformed Accept ALSO → JSON (never a stream)', async () => {
    const h = harness(createInMemoryTurnSubmittedSink());
    await driveCreate(h, 'c-1');

    // No Accept header → the plain S3 result object (NOT an sseResponse).
  ...
  it('TRUST-BOUNDARY INTAKE-FRAME FRAMING: a hostile message_id (raw CR/LF) cannot break the conversation_intake frame — its data stays one JSON-escaped line that round-trips the id intact', async () => {
    ...
    // payload is otherwise all safe-id/number fields, so SS-1 (which drives the reply text) never
    // covered it (the FQ-1 gap). A BENIGN terminal reply keeps this arm isolated to the intake builder.
    const h = harness(createInMemoryTurnSubmittedSink(), {
  ...
  it('TRUST-BOUNDARY ERROR-FRAME FRAMING: an unexpected reply-leg throw + a hostile message_id cannot break the conversation_reply_error frame — its data stays one JSON-escaped line', async () => {
    ...
    // SS-3 unexpected throw → the producer emits a terminal conversation_reply_error frame CARRYING
    // the committed intake, whose message_id is hostile — driving hostile bytes through the ERROR
    // builder (SS-1 only ever asserted BENIGN content there — the second half of the FQ-1 gap).
  ...
  it('SS-2 the client-stream forwarder is text_delta-ONLY: tool/reasoning/lifecycle events NEVER reach the client (they stay durable in run_events)', async () => {
    const h = harness(createInMemoryTurnSubmittedSink());
  ...
  it('PBC-1 terminal-frame usage: a FRESH streamed reply carries usage EQUAL to the fresh JSON path; a ledger-served re-POST has NO usage in EITHER representation', async () => {
    const h = harness(createInMemoryTurnSubmittedSink());
    await driveCreate(h, 'c-1');
    ...
    // AND the terminal frame (the S3 residual: the run header stores no token counts to re-derive).
  ...
  it('SS-3 an UNEXPECTED reply-leg throw still streams a terminal conversation_reply_error (JSON-500 symmetry) — the stream closes cleanly, no hang', async () => {
    const sink = createInMemoryTurnSubmittedSink();
    const h = harness(sink);
  ...
  it('S4-LOR-MINOR-2 content-negotiation honors q-values: text/event-stream;q=0 → JSON; a positive-q SSE range → stream; malformed → JSON (never a 500)', async () => {
    const h = harness(createInMemoryTurnSubmittedSink());
    await driveCreate(h, 'c-1');
