# @rayspec/capability-bridges

The durable-workflow ↔ capability **composition seams** for the four ingress
capabilities, merged into one package. Each sub-barrel re-exports its capability's
event → `WorkflowInputEvent` adapter plus the fail-closed `*Sink` a deployment injects
to enqueue a durable workflow run on ingress.
