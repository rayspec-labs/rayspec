...
export async function runGenSecrets(args: readonly string[]): Promise<GenSecretsResult> {
  ...
  // Hold ONE file handle for the whole read-decide-write cycle — no `existsSync` check-then-write race.
  // `wx+` atomically creates the file when absent (⇒ fresh, header written); an `EEXIST` means it already
  // exists ⇒ reopen `r+` and read the operator's current content through the SAME handle.
  ...
  try {
    const present = existing === null ? new Set<SecretKey>() : presentKeys(existing);
    ...
    if (newLines.length > 0) {
      ...
      // Write the delta at end-of-file (a fresh file ⇒ offset 0), then tighten the SAME fd to owner-only.
      await handle.write(payload, existing === null ? 0 : Buffer.byteLength(existing), 'utf8');
