...
export async function runGenSecrets(args: readonly string[]): Promise<GenSecretsResult> {
  ...
  if (newLines.length > 0) {
    if (existing === null) {
      ...
      writeFileSync(target, `${header}${newLines.join('\n')}\n`, 'utf8');
    } else {
      // APPEND only the missing keys — never rewrite or reorder the operator's existing content.
      const sep = existing.length === 0 || existing.endsWith('\n') ? '' : '\n';
      writeFileSync(target, `${existing}${sep}${newLines.join('\n')}\n`, 'utf8');
    }
    // Tighten to owner-only whenever we have written secret material into the file.
