...
export const codeClaim = async (args, init) => {
  ...
  try {
    ...
    const clamped = [];
    {
      ...
      const ORDER = ["ok", "review", "violation"];
      const proposed = coerced.row.policy_flag;
      if (typeof proposed === 'string' && ORDER.indexOf(proposed) > 1) {
        coerced.row.policy_flag = "review";
        clamped.push({ column: "policy_flag", proposed, applied: "review" });
      }
    }
