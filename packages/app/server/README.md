...
| `RAYSPEC_SKIP_DOTENV` | no | Set to `1` to skip the local-DX `.env` loader (prove a pure-ambient-env boot). That loader reads `$PWD/.env` first and the checkout-root `.env` second, per key, and never overrides a variable already set. |
