...
describe('the floor covers every path prefix the composition root injects', () => {
  it('covers the probe half for a document with no mounts', () => {
    ...
    for (const injected of platformPublicRoutePrefixes()) expect(floor).toContain(injected);
  });

  it('covers the declared-mount half too, root mount included (it injects none)', () => {
    const floor = reservedApiPathPrefixes(MOUNTS);
    for (const injected of platformPublicRoutePrefixes(MOUNTS)) expect(floor).toContain(injected);
    // The root mount is absent from BOTH sides — the exemption is one decision, made the same way.
    expect(platformPublicRoutePrefixes(MOUNTS)).not.toContain('/');
  ...
  it('the two sides answer the same way about a route path', () => {
    ...
    for (const claimed of [HEALTH_PATH, `${RECOVERY_SCOPE_PATH}/deep`, '/app', '/app/config']) {
      ...
      expect(isReservedApiPath(claimed, platformPublicRoutePrefixes(MOUNTS))).toBe(true);
    }
    ...
    expect(isReservedApiPath('/healthy', platformPublicRoutePrefixes(MOUNTS))).toBe(false);
  });
  ...
  it('and describe it in the same words — the floor and the boot list the same prefixes', () => {
    ...
    const bootPrefixes = ['/v1/', '/oidc/', ...platformPublicRoutePrefixes(MOUNTS)];
    expect(reservedRoutePathRefusal('GET', '/app/config', reservedApiPathPrefixes(MOUNTS))).toBe(
      reservedRoutePathRefusal('GET', '/app/config', bootPrefixes),
    );
  });
});
