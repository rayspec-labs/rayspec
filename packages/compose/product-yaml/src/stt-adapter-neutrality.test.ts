 ...
 *
 * The adapter source is byte-stable, so a fail-the-fix that would need to MUTATE it is proven instead
...
describe('STT provider-neutral public surface (structural allowlist)', () => {
  ...
  it('the allowlist has teeth: a synthetic leaked provider-native field is caught by the exact-set diff', () => {
    // Cannot mutate the byte-stable adapter surface, so prove the diff on a SYNTHETIC leaky surface.
    const leakySurface = [
