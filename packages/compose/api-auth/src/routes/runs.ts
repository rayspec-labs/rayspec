...
/**
 * REPLAY a run's events from run_events as SSE — the run surface's binding of the platform's journal
 * replay (`http/journal-replay.ts`), which owns the bound, the order and the resume cursor. ONE-SHOT,
 * not a live tail: for a SYNC run the run is already complete by the time anyone reads /events, so
 * this returns the whole stream; for an ASYNC (off-request) run the client pulls newly-persisted
 * events by RE-REQUESTING from the last seq it saw.
 *
 * What this route contributes is the READ-SIDE VOCABULARY: `serializeEventData` re-parses each stored
 * jsonb `data` as a neutral `NeutralEvent` and drops any row that is not one, so a poisoned row can
 * never leak a non-neutral payload to the client.
 */
