...
The Tier-A **durable workflow execution engine**. Wires the reviewed
`@rayspec/foundation` onto the real platform primitives — TenantDb journal, the DBOS
durable path, failure semantics, and the real agent/capability/validate/store nodes.
Product-free: all product meaning comes from the validated spec.
